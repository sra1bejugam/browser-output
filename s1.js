...
const fs = require('fs');
prompt.start();
prompt.get(['Filename','Teams'], function (err, result) {
    ...
    console.log(`Please enter integers between one and ${info}`);
    }
});
let server=http.createServer(function(req,res){
    console.log('request had made:'+req.url);
    if(req.url==='/'){
    res.writeHead(200,{'Content-Type':'text/plain'});
    let read=fs.createReadStream(__dirname+'/results.txt','utf-8');
    read.pipe(res);
